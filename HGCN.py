...
def load_st_construct_H(
    ...
    if True:
        ...
        student_feature = pd.read_csv("data/student_features_min_oversampler.csv")
        ...
        fts_info= fts_[:,1:164].astype(np.float32)#info 1
        fts_breakfast = fts_[:,164:174].astype(np.float32)#breakfast 1
        fts_lunch = fts_[:,174:184].astype(np.float32) #lunch 1
        fts_dinner = fts_[:,184:194].astype(np.float32) #dinner
        fts_wg = fts_[:,194:212].astype(np.float32)#wg
        fts_wljf = fts_[:,212:231].astype(np.float32)#wljf
        fts_library = fts_[:,231:235].astype(np.float32)#library
        fts_shopping = fts_[:,235:242].astype(np.float32) #shopping
    ...
    if use_st_feature_for_structure:
        tmp = hgut.construct_muiH_with_KNN(fts_info,fts_breakfast,fts_lunch,fts_dinner,fts_wg,fts_wljf,fts_library,fts_shopping,lbls, K_neigs=K_neigs,
