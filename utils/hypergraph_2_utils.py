...
def construct_muiH_with_KNN(fts_info,fts_library,fts_breakfast,fts_lunch,fts_dinner, y, K_neigs=[2], split_diff_scale=False, is_probH=True, m_prob=1):
    """
    init multi-scale hypergraph Vertex-Edge matrix from original node feature matrix
    :param X: N_object x feature_number
    :param K_neigs: the number of neighbor expansion
...
#             H_shopping.append(H_tmp)
    ...
    H = hyperedge_concat(H_info,H_library,H_breakfast,H_lunch,H_dinner)
     
    return H

def construct_duibiH_with_KNN(fts_info,fts_library,fts_breakfast,fts_lunch,fts_dinner,y, K_neigs, split_diff_scale=False, is_probH=True, m_prob=1):
    """
    init multi-scale hypergraph Vertex-Edge matrix from original node feature matrix
    :param X: N_object x feature_number
    :param K_neigs: the number of neighbor expansion
